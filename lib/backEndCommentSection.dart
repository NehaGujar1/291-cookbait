      ...
      if (element.id == recipeID) {
        ...
        for (int i = 0; i < listOfFields.length; i++) {
          if (listOfFields[i.toString()][0] == email) {
            final documentToBeUpdated =
                FirebaseFirestore.instance.collection('comments').doc(recipeID);
            documentToBeUpdated.update(
              {
                i.toString(): FieldValue.arrayUnion([comment]),
              },
            );
